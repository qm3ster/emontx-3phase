...
#include <rfm69nTxLib.h>


/*
Interface for the RFM69CW Radio Module in "RFM69 Native" mode, with Channel Busy detection.
*/
...
bool rfm_send(const byte *data, const byte size, const byte group, const byte node, const uint8_t rf_freq, const byte rf_power, const int threshold, const byte timeout) 
{
  ...
  writeReg(REG_DIOMAPPING1, 0x00); 										                 // PacketSent
	...
	volatile uint8_t txstate = 0, flags = 0, parity;
	byte i = 0;
	...
	while(txstate < 4)
	{
		if ((readReg(REG_IRQFLAGS2) & IRQ2_FIFOFULL) == 0)		             // FIFO !full
		{
			...
			switch(txstate)
			{
        case 0: next = size+2; txstate++; break;				               // No. of payload bytes ('size' includes NodeID & parity but not itself)
        case 1: parity = group ^ (group << 4);
                parity = (parity ^ (parity << 2)) & 0xC0;
                next = (flags & 0x3F) | parity; txstate++; break;      // Flags
        case 2: next = ((flags & 0xC0) | (node & 0x3F)); txstate++; break;      // Flags(2bits), NodeID (6bits)
        case 3: next=data[i++]; if(i==size) txstate++; break;
			}
...
{   
	...
	while (!(readReg(REG_IRQFLAGS2) & IRQ2_PACKETSENT))				           // wait for transmission to complete (not present in JeeLib) 
	    delay(1);
	writeReg(REG_OPMODE, (readReg(REG_OPMODE) & 0xE3) | 0x00); 		       // Sleep Mode	
}
...
bool rfm_send(const byte *data, const byte size, const byte group, const byte node, const uint8_t rf_freq, const byte rf_power, const int threshold, const byte timeout)
{
	byte i=0,j=0,next,txstate=0,flags=0,parity;
	word crc=0x1D0F;
  ...
  if (timeout)
  {
	...
	while(txstate<14)
	{
		...
		switch(txstate)
		{
			...
			case 5: next=size+2; txstate++; break; // 'size' includes NodeID & parity but not itself
      case 6: parity = group ^ (group << 4);
              parity = (parity ^ (parity << 2)) & 0xC0;
              next = (flags & 0x3F) | parity; txstate++; break; // Flags
      case 7: next = ((flags & 0xC0) | (node & 0x3F)); txstate++; break; // Flags(2bits), NodeID (6bits)
 			case 8: next=data[i++]; if(i==size) txstate++; break;
			case 9: next=(byte)(~crc>>8); txstate++; break;
			case 10: next=(byte)~crc; txstate++; break;
			case 11:
			case 12:
			case 13: next=0xaa; txstate++; break; // dummy bytes (if <3 CRC gets corrupted sometimes)
		}
		if((txstate>5)&&(txstate<10)) 
      crc = _crc_xmodem_update(crc, next);
    if((txstate>5)&&(txstate<12)) 
      next ^= whitening[j++];
